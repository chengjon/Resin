...
func (r *StateRepo) UpsertPlatform(p model.Platform) error {
	...
	_, err = r.db.Exec(`
		INSERT INTO platforms (id, name, sticky_ttl_ns, regex_filters_json, region_filters_json,
		                       ...
		                       reverse_proxy_fixed_account_header, allocation_policy, require_egress_ip,
		                       require_latency, updated_at_ns)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
			name                     = excluded.name,
			...
			require_egress_ip        = excluded.require_egress_ip,
			require_latency          = excluded.require_latency,
			updated_at_ns            = excluded.updated_at_ns
		...
		p.AllocationPolicy, p.RequireEgressIP, p.RequireLatency, p.UpdatedAtNs)
	if err != nil {
...
func (r *StateRepo) GetPlatform(id string) (*model.Platform, error) {
	...
	if err := row.Scan(&p.ID, &p.Name, &p.StickyTTLNs, &regexFiltersJSON,
		...
		&p.ReverseProxyFixedAccountHeader, &p.AllocationPolicy, &p.RequireEgressIP,
		&p.RequireLatency, &p.UpdatedAtNs); err != nil {
		if err == sql.ErrNoRows {
...
func (r *StateRepo) ListPlatforms() ([]model.Platform, error) {
	rows, err := r.db.Query("SELECT id, name, sticky_ttl_ns, regex_filters_json, region_filters_json, reverse_proxy_miss_action, reverse_proxy_empty_account_behavior, reverse_proxy_fixed_account_header, allocation_policy, require_egress_ip, require_latency, updated_at_ns FROM platforms")
	if err != nil {
		return nil, err
	}
	defer rows.Close()
